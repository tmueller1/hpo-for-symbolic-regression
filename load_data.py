...
#        Y = y
#        var = IntVar()
#        Label_self = Label(frame,text = text)
#        Label_self.place(x = X, y = Y)
#
#        toggle_btn = Checkbutton(frame,text = 'info?',
#            command=lambda: self.toggle(var, toggle_btn, Label_self),
#            variable=var
#                                )
#        toggle_btn.pack()
...
def direc() :
    default_dir = r"abc"
    fname = tkinter.filedialog.askopenfilename(title=u'abc', initialdir=(os.path.expanduser((default_dir))))
    desktop_path = ''
    ...
    for colindex in range(0,oldWbS.ncols) :
        if(type(oldWbS.cell(1,colindex).value) == type("a")):
            ...
            for rowIndex in range(0,oldWbS.nrows):
                newWs.write(rowIndex,step,oldWbS.cell(rowIndex,colindex).value)
            step = step + 1
...
def solv() :
    ...
    print(formu)
    print(fit)
    formula = tkinter.Label(
        master=frame1,
        text= "Formular \n" + formu,
        background="black",
        fg="white"
    )
    fitness = tkinter.Label(
        master=frame1,
        text="RMSE \n" + fit,
        background="black",
        fg="white"
    )
    fitness2 = tkinter.Label(
        master=frame1,
        text="Multifitness \n" + fit2,
        background="black",
        fg="white"
...
if __name__ == '__main__':
    ...
    num_in_gen = tkinter.Label(
        master=frame2,
        text="Population size",
        foreground="white",
        background="black"
    )
    Num_of_gen = tkinter.Label(
        master=frame2,
    ...
    entry_num_in_gen = tkinter.Entry(
        master=frame2
    )
    ...
    Info_generation = tkinter.Label(
        ...
        text="with more generation is more likely  \n"
             ...
             "take more time by default is 25\n"
    ...
    Info_cxpb = tkinter.Label(
        master=frame2,
        text="The probability of mating two individuals and mutating an individual\n"
    ...
    def check_lenth():
        print("now the var_lent")
        print(var_lenth.get())
        if(var_lenth.get() == 1):
            Info_lenth.place(x = 280,y = 90)
        else :
            Info_lenth.place_forget()
    len_btn = tkinter.Checkbutton(master=frame2,text = "info?",variable = var_lenth,command = check_lenth,onvalue = 1,offvalue = 0)
    var_num_in_gen = tkinter.IntVar()
    def check_num_in_gen():
        print("now the num_in_gen")
        print(var_num_in_gen.get())
        if (var_num_in_gen.get() == 1):
            Info_num_in_gen.place(x=1150, y=90)
        else:
            Info_num_in_gen.place_forget()

    num_in_gen_btn = tkinter.Checkbutton(master=frame2, text="info?", variable=var_num_in_gen, command=check_num_in_gen, onvalue=1,
                                  offvalue=0)
    var_gen = tkinter.IntVar()
    def check_gen():
        print("now the gen_lent")
        print(var_gen.get())
        if (var_gen.get() == 1):
    ...
    gen_btn = tkinter.Checkbutton(master=frame2, text="info?", variable=var_gen, command=check_gen, onvalue=1,
                                  offvalue=0)
    var_cxpb = tkinter.IntVar()


    def check_cxpb():
        print("now the cxpb_lent")
        print(var_cxpb.get())
        if (var_cxpb.get() == 1):
    ...
    cxpb_btn = tkinter.Checkbutton(master=frame2, text="info?", variable=var_cxpb, command=check_cxpb, onvalue=1,
                                  offvalue=0)
    var_loadin = tkinter.IntVar()


    def check_loadin():
        print("now the loadin_lent")
        print(var_loadin.get())
        if (var_loadin.get() == 1):
    ...
    loadin_btn = tkinter.Checkbutton(master=frame2, text="info?", variable=var_loadin, command=check_loadin, onvalue=1,
                                   offvalue=0)
    frame2.pack()
    frame1.pack()
